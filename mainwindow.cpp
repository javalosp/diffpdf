...
void MainWindow::createConnections()
{
    ...
    connect(setFile1Button, &QAbstractButton::clicked, this, [this]
            { setFile1(); });
    connect(setFile2Button, &QAbstractButton::clicked, this, [this]
            { setFile2(); });
                                                    ...
                                                    const PagePair &pair) const
{
    ...
    if (zoningGroupBox->isChecked())
        zoning = QString("%1:%2:%3").arg(columnsSpinBox->value()).arg(toleranceRSpinBox->value()).arg(toleranceYSpinBox->value());
    QString margins;
    if (marginsGroupBox->isChecked())
        margins = QString("%1:%2:%3:%4").arg(topMarginSpinBox->value()).arg(bottomMarginSpinBox->value()).arg(leftMarginSpinBox->value()).arg(rightMarginSpinBox->value());
    const QString key = QString("%1:%2:%3:%4:%5").arg(index).arg(zoomSpinBox->value()).arg(comparisonMode).arg(zoning).arg(margins);
    const QString key1 = QString("1:%1:%2:%3").arg(key).arg(pair.left).arg(filename1LineEdit->text());
    const QString key2 = QString("2:%1:%2:%3").arg(key).arg(pair.right).arg(filename2LineEdit->text());
                                               ...
                                               const PdfDocument &pdf2)
{
    ...
    while (!pages1.isEmpty() && !pages2.isEmpty())
    {
        ...
        statusLabel->setText(tr("Comparing %1/%2").arg(++number).arg(total));
    }
                                 ...
                                 const int millisec)
{
    ...
    if (differ == 1) // Separated the cases for ease of translation
        statusLabel->setText(tr("1 differs %1/%2 compared").arg(pair.first).arg(pair.second));
    else
        statusLabel->setText(tr("%1 differ %2/%3 compared").arg(differ).arg(pair.first).arg(pair.second));
